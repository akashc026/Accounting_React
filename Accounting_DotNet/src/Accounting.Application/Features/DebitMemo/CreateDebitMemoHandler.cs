...
namespace Accounting.Application.Features
{
    public class CreateDebitMemoHandler : CreateEntityHandler<AccountingDbContext, DebitMemo, Guid, CreateDebitMemo, Guid>
    {
        ...
        public CreateDebitMemoHandler(AccountingDbContext dbContext, IMapper mapper, IFormSequenceService formSequenceService)
            : base(dbContext, mapper)
        {
            _formSequenceService = formSequenceService;
        }

        protected override async Task<DebitMemo> CreateEntityAsync(CreateDebitMemo request, IMapper mapper, CancellationToken cancellationToken)
        {
