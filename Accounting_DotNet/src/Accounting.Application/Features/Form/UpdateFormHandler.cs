...
namespace Accounting.Application.Features
{
    public class UpdateFormHandler : IRequestHandler<UpdateForm, Guid>
    {
        ...
        public async Task<Guid> Handle(UpdateForm request, CancellationToken cancellationToken)
        {
            ...
            if (request.TypeOfRecord.HasValue && request.TypeOfRecord.Value != Guid.Empty)
            {
                entity.TypeOfRecord = request.TypeOfRecord.Value;
            }
            ...
            // Update Inactive if provided
            if (request.Inactive.HasValue)
            {
                entity.Inactive = request.Inactive.Value;
            }

            // Only update account fields if they have valid values (not null and not empty Guid)
