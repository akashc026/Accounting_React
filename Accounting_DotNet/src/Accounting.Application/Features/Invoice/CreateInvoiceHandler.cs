...
namespace Accounting.Application.Features
{
    public class CreateInvoiceHandler : CreateEntityHandler<AccountingDbContext, Invoice, Guid, CreateInvoice, Guid>
    {
        ...
        public CreateInvoiceHandler(AccountingDbContext dbContext, IMapper mapper, IFormSequenceService formSequenceService)
            : base(dbContext, mapper)
        {
            _formSequenceService = formSequenceService;
        }

        protected override async Task<Invoice> CreateEntityAsync(CreateInvoice request, IMapper mapper, CancellationToken cancellationToken)
        {
