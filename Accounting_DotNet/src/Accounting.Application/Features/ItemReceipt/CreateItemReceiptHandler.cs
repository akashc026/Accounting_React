...
namespace Accounting.Application.Features
{
    public class CreateItemReceiptHandler : CreateEntityHandler<AccountingDbContext, ItemReceipt, Guid, CreateItemReceipt, Guid>
    {
        ...
        public CreateItemReceiptHandler(AccountingDbContext dbContext, IMapper mapper, IFormSequenceService formSequenceService)
            : base(dbContext, mapper)
        {
            _formSequenceService = formSequenceService;
        }

        protected override async Task<ItemReceipt> CreateEntityAsync(CreateItemReceipt request, IMapper mapper, CancellationToken cancellationToken)
        {
