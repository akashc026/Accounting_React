...
namespace Accounting.Application.Features
{
    public class CreateJournalEntryHandler : CreateEntityHandler<AccountingDbContext, JournalEntry, Guid, CreateJournalEntry, Guid>
    {
        ...
        public CreateJournalEntryHandler(AccountingDbContext dbContext, IMapper mapper, IFormSequenceService formSequenceService) : base(dbContext, mapper)
        {
            _formSequenceService = formSequenceService;
        }

        protected override async Task<JournalEntry> CreateEntityAsync(CreateJournalEntry request, IMapper mapper, CancellationToken cancellationToken)
        {
