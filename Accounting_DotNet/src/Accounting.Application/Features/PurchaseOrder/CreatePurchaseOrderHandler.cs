...
namespace Accounting.Application.Features
{
    public class CreatePurchaseOrderHandler : CreateEntityHandler<AccountingDbContext, PurchaseOrder, Guid, CreatePurchaseOrder, Guid>
    {
        ...
        public CreatePurchaseOrderHandler(AccountingDbContext dbContext, IMapper mapper, IFormSequenceService formSequenceService) : base(dbContext, mapper)
        {
            _formSequenceService = formSequenceService;
        }

        protected override async Task<PurchaseOrder> CreateEntityAsync(CreatePurchaseOrder request, IMapper mapper, CancellationToken cancellationToken)
        {
