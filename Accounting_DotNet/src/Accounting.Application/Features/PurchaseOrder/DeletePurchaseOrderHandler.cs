...
namespace Accounting.Application.Features
{
    public class DeletePurchaseOrderHandler : DeleteEntityHandler<AccountingDbContext, PurchaseOrder, Guid, DeletePurchaseOrder>
    {
        ...
        public override async Task<Unit> Handle(DeletePurchaseOrder request, CancellationToken cancellationToken)
        {
            var entity = await Entities.IgnoreQueryFilters().FirstOrDefaultAsync(x => x.Id == request.Id, cancellationToken);
            if (entity == null)
            {
                throw new KeyNotFoundException($"PurchaseOrder with Id of {request.Id} was not found.");
            }
            ...
            await DbContext.Procedures.CheckBlockingReferencesAsync(
                ...
                excludeTables: "PurchaseOrderLines,ItemReceipts",
                totalCount: totalCount,
                cancellationToken: cancellationToken);

            if ((totalCount.Value ?? 0) > 0)
            {
                throw new InvalidOperationException("PurchaseOrder delete not allowed due to existing references.");
            }
        ...
        {
            return DbContext.Model.FindEntityType(typeof(PurchaseOrder))?.GetTableName() ?? nameof(PurchaseOrder);
        }

        private async Task SoftDeleteLinesAsync(Guid purchaseOrderId, CancellationToken cancellationToken)
        {
            var lines = await DbContext.PurchaseOrderLines
                .IgnoreQueryFilters()
                .Where(x => x.POID == purchaseOrderId)
            ...
            if (lines.Count > 0)
            {
                DbContext.PurchaseOrderLines.UpdateRange(lines);
            }
        }
    }
}
