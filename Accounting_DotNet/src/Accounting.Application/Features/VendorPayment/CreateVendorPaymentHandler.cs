...
namespace Accounting.Application.Features
{
    public class CreateVendorPaymentHandler : CreateEntityHandler<AccountingDbContext, VendorPayment, Guid, CreateVendorPayment, Guid>
    {
        ...
        public CreateVendorPaymentHandler(AccountingDbContext dbContext, IMapper mapper, IFormSequenceService formSequenceService) : base(dbContext, mapper)
        {
            _formSequenceService = formSequenceService;
        }

        protected override async Task<VendorPayment> CreateEntityAsync(CreateVendorPayment request, IMapper mapper, CancellationToken cancellationToken)
        {
