...
const InventoryAdjustmentList = () => {
  ...
  // Get location name
  const getLocationName = (adjustment) => {
    
    // Use locationName from API response if available
    if (adjustment.locationName) {
      return adjustment.locationName;
    }
    // Fallback if locationName not in response
    return adjustment.locationName || 'Unknown Location';
  };
  ...
  // Custom cell for location display
  const LocationCell = (props) => {
    return (
      <td {...props.tdProps}>
        {getLocationName(props.dataItem)}
      </td>
    );
  };
