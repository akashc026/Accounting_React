...
const InventoryTransferList = () => {
  ...
  const getFromLocationName = (transfer) => {
    if (transfer.fromLocationName) {
      return transfer.fromLocationName;
    }
    return transfer.fromLocation || 'Unknown';
  };

  // Get to location name
  const getToLocationName = (transfer) => {
    if (transfer.toLocationName) {
      return transfer.toLocationName;
    }
    return transfer.toLocation || 'Unknown';
  };
  ...
  // Custom cell for from location display
  const FromLocationCell = (props) => {
    return (
      <td {...props.tdProps}>
        {getFromLocationName(props.dataItem)}
      </td>
    );
  };

  // Custom cell for to location display
  const ToLocationCell = (props) => {
    return (
      <td {...props.tdProps}>
        {getToLocationName(props.dataItem)}
      </td>
    );
  };
