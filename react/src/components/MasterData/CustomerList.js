...
const CustomerList = () => {
  ...
  const ActionCell = (props) => {
    const customer = props.dataItem;
    if (!customer || !customer.id) {
      return <td {...props.tdProps}>Invalid customer data</td>;
    }
  ...
  if (error) {
    return (
      <div className="k-p-4">
        <div className="customer-list-container error-container">
      ...
      <div className="customer-list-container">
        ...
        {(!data?.results || data.results.length === 0) ? (
          <div className="no-data-message">
            <p>No customers found. Click the "Add New Customer" button above to create your first customer.</p>
          </div>
        ) : processedData().length === 0 ? (
          <div className="no-data-message">
            <p>No customers match your current search criteria. Try adjusting your filters.</p>
          </div>
        ) : (
      <Grid
            data={processedData()}
