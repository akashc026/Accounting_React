...
import { useCustomerPayments, useCustomers, useStatus } from '../../hooks/useMasterData';
...
const CustomerPaymentList = () => {
  ...
  const { customerPayments, loading, error, refresh, fetchWithPagination } = useCustomerPayments();
  ...
  const fetchData = useCallback(async (pageNumber = 1, pageSize = 10, search = '', status = 'All', sortField = null, sortDir = null) => {
    try {
      ...
      const url = buildUrl(`/customer-payment?${queryParams}`);
      const response = await fetch(url);

      if (!response.ok) {
        throw new Error(`Failed to fetch customer payments: ${response.status}`);
      }
  ...
  const handleRefresh = () => {
    ...
    const sortDir = gridData.sort?.[0]?.dir || null;
    fetchData(pageNumber, pageSize, debouncedSearchText, statusFilter, sortField, sortDir);
    showNotification('Customer payments refreshed successfully', 'success');
  };

  // Process customer payments data for the Grid (server-side filtering)
  ...
  const handleView = (id) => {
    navigate(`/customer-payment/view/${id}`);
  };

  // Handle Edit button click
  const handleEdit = (id) => {
    navigate(`/customer-payment/edit/${id}`);
  };
  ...
  const ActionCell = (props) => {
    const payment = props.dataItem;
    if (!payment || !payment.id) {
      return <td {...props.tdProps}>Invalid payment data</td>;
    }

    const isClosed = payment.status === 'b2dae51c-12b6-4a90-adfd-4e2345026b70';
  ...
  if (error) {
    return (
      <div className="k-p-4">
        <div className="payment-list-container error-container">
          <div className="error-message">
            <div>Error loading customer payments: {error}</div>
    ...
    <div className="k-p-4">
      <div className="payment-list-container">
        ...
        <div className="list-header">
          <h2>Customer Payment Master</h2>
        <Button
          onClick={() => navigate('/customer-payment/new')}
          className="k-button k-button-md k-rounded-md k-button-solid k-button-solid-primary"
          >
            <FaPlus /> Add New Customer Payment
          </Button>
        </div>
        ...
        {loading ? (
          ...
          <div className="no-data-message">
            <p>No customer payments found. Click the "Add New Customer Payment" button above to create your first customer payment.</p>
          </div>
        ) : processedData().length === 0 ? (
          <div className="no-data-message">
            <p>No customer payments match your current search criteria. Try adjusting your filters.</p>
          </div>
        ) : (
      <Grid
            data={processedData()}
