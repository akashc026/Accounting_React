...
const ItemReceiptList = () => {
  ...
  const fetchData = useCallback(async (pageNumber = 1, pageSize = 10, search = '', status = 'All', sortField = null, sortDir = null) => {
    try {
      ...
      const url = buildUrl(`/item-receipt?${queryParams}`);
      const response = await fetch(url);

      if (!response.ok) {
        throw new Error(`Failed to fetch item receipts: ${response.status}`);
      }
      ...
      console.error('Error fetching item receipts:', err);
      setData({ results: [] });
      setTotalItems(0);
    }
  }, []);
