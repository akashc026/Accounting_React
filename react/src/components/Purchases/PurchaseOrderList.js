...
const PurchaseOrderList = () => {
  ...
  const { loading, error, refresh, fetchWithPagination } = purchaseOrdersHook;
  ...
  const fetchData = useCallback(async (pageNumber = 1, pageSize = 10, search = '', status = 'All', sortField = null, sortDir = null) => {
    try {
      ...
      const url = buildUrl(`/purchase-order?${queryParams}`);
  ...
  // Process purchase orders data for the Grid (server-side filtering)
  ...
  const handleRefresh = () => {
    ...
    const sortDir = gridData.sort?.[0]?.dir || null;
    fetchData(pageNumber, pageSize, debouncedSearchText, statusFilter, sortField, sortDir);
    showNotification('Purchase orders refreshed successfully', 'success');
  };
  ...
  if (error) {
    return (
      <div className="k-p-4">
        <div className="purchase-order-list-container error-container">
          <div className="error-message">
            <div>Error loading purchase orders: {error}</div>
      ...
      <div className="purchase-order-list-container">
        ...
        {(!data?.results || data.results.length === 0) ? (
          <div className="no-data-message">
            <p>No purchase orders found. Click the "Add New Purchase Order" button above to create your first purchase order.</p>
          </div>
        ) : processedData().length === 0 ? (
          <div className="no-data-message">
            <p>No purchase orders match your current search criteria. Try adjusting your filters.</p>
          </div>
        ) : (
          <Grid
            data={processedData()}
