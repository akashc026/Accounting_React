...
import { useVendorBills, useVendors, useStatus } from '../../hooks/useMasterData';
...
const VendorBillList = () => {
  ...
  const { vendorBills, loading, error, refresh, fetchWithPagination } = useVendorBills();
  ...
  const fetchData = useCallback(async (pageNumber = 1, pageSize = 10, search = '', status = 'All', sortField = null, sortDir = null) => {
    try {
      ...
      const url = buildUrl(`/vendor-bill?${queryParams}`);
      const response = await fetch(url);

      if (!response.ok) {
        throw new Error(`Failed to fetch vendor bills: ${response.status}`);
      }
  ...
  // Process vendor bills data for the Grid (server-side filtering)
  ...
  const handleView = (id) => {
    navigate(`/vendor-bill/view/${id}`);
  };

  // Handle Edit button click
  const handleEdit = (id) => {
    navigate(`/vendor-bill/edit/${id}`);
  };
  ...
  const handleRefresh = () => {
    ...
    const sortDir = gridData.sort?.[0]?.dir || null;
    fetchData(pageNumber, pageSize, debouncedSearchText, statusFilter, sortField, sortDir);
    showNotification('Vendor bills refreshed successfully', 'success');
  };
  ...
  const ActionCell = (props) => {
    const vendorBill = props.dataItem;
    if (!vendorBill || !vendorBill.id) {
      return <td {...props.tdProps}>Invalid vendor bill data</td>;
    }

    const isClosed = vendorBill.status === 'b2dae51c-12b6-4a90-adfd-4e2345026b70';

    return (
      <td {...props.tdProps} className="k-command-cell">
        <div className="action-buttons">
        <Button
            onClick={() => handleView(vendorBill.id)}
        ...
        {!isClosed && (
          <Button
              onClick={() => handleEdit(vendorBill.id)}
  ...
  if (loading) {
    return (
      <div className="k-p-4">
        <div className="vendor-bill-list-container">
          <div className="loading-indicator">
            <div className="spinner"></div>
            <div>Loading vendor bills...</div>
          </div>
        </div>
      </div>
    );
  ...
  if (error) {
    return (
      <div className="k-p-4">
        <div className="vendor-bill-list-container error-container">
          <div className="error-message">
            <div>Error loading vendor bills: {error}</div>
  ...
  return (
    <div className="k-p-4">
      <div className="vendor-bill-list-container">
        ...
        <Button
          onClick={() => navigate('/vendor-bill/new')}
          className="k-button k-button-md k-rounded-md k-button-solid k-button-solid-primary"
          >
            <FaPlus /> Add New Vendor Bill
          </Button>
        </div>
        ...
        {(!data?.results || data.results.length === 0) ? (
          <div className="no-data-message">
            <p>No vendor bills found. Click the "Add New Vendor Bill" button above to create your first vendor bill.</p>
          </div>
        ) : processedData().length === 0 ? (
          <div className="no-data-message">
            <p>No vendor bills match your current search criteria. Try adjusting your filters.</p>
          </div>
        ) : (
      <Grid
            data={processedData()}
