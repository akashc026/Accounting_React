...
import { useVendorCredits, useVendors, useStatus } from '../../hooks/useMasterData';
...
const VendorCreditList = () => {
  ...
  const { vendorCredits, loading, error, refresh, fetchWithPagination } = useVendorCredits();
  ...
  const fetchData = useCallback(async (pageNumber = 1, pageSize = 10, search = '', status = 'All', sortField = null, sortDir = null) => {
    try {
      ...
      const url = buildUrl(`/vendor-credit?${queryParams}`);
      ...
      console.error('Error fetching vendor credits:', err);
      setData({ results: [] });
      setTotalItems(0);
    }
  }, []);
  ...
  // Process vendor credits data for the Grid (server-side filtering)
  ...
  const handleView = (id) => {
    navigate(`/vendor-credit/view/${id}`);
  };

  // Handle Edit button click
  const handleEdit = (id) => {
    navigate(`/vendor-credit/edit/${id}`);
  };
  ...
  const handleRefresh = () => {
    ...
    const sortDir = gridData.sort?.[0]?.dir || null;
    fetchData(pageNumber, pageSize, debouncedSearchText, statusFilter, sortField, sortDir);
    showNotification('Vendor credits refreshed successfully', 'success');
  };
  ...
  const ActionCell = (props) => {
    const vendorCredit = props.dataItem;
    if (!vendorCredit || !vendorCredit.id) {
      return <td {...props.tdProps}>Invalid vendor credit data</td>;
    }

    const isClosed = vendorCredit.status === 'b2dae51c-12b6-4a90-adfd-4e2345026b70';

    return (
      <td {...props.tdProps} className="k-command-cell">
        <div className="action-buttons">
        <Button
            onClick={() => handleView(vendorCredit.id)}
        ...
        {!isClosed && (
          <Button
              onClick={() => handleEdit(vendorCredit.id)}
  ...
  if (loading) {
    return (
      <div className="k-p-4">
        <div className="vendor-credit-list-container">
          <div className="loading-indicator">
            <div className="spinner"></div>
            <div>Loading vendor credits...</div>
          </div>
        </div>
      </div>
    );
  ...
  if (error) {
    return (
      <div className="k-p-4">
        <div className="vendor-credit-list-container error-container">
          <div className="error-message">
            <div>Error loading vendor credits: {error}</div>
    ...
    <div className="k-p-4">
      <div className="vendor-credit-list-container">
        ...
        <div className="list-header">
          <h2>Vendor Credit Master</h2>
        <Button
          onClick={() => navigate('/vendor-credit/new')}
          className="k-button k-button-md k-rounded-md k-button-solid k-button-solid-primary"
          >
            <FaPlus /> Add New Vendor Credit
          </Button>
        </div>
        ...
        {(!data?.results || data.results.length === 0) ? (
          <div className="no-data-message">
            <p>No vendor credits found. Click the "Add New Vendor Credit" button above to create your first vendor credit.</p>
          </div>
        ) : processedData().length === 0 ? (
          <div className="no-data-message">
            <p>No vendor credits match your current search criteria. Try adjusting your filters.</p>
          </div>
        ) : (
      <Grid
            data={processedData()}
