...
import { useCreditMemos, useCustomers, useStatus } from '../../hooks/useMasterData';
...
const CreditMemoList = () => {
  ...
  const { creditMemos, loading, error, refresh, fetchWithPagination } = useCreditMemos();
  ...
  const fetchData = useCallback(async (pageNumber = 1, pageSize = 10, search = '', status = 'All', sortField = null, sortDir = null) => {
    try {
      ...
      const url = buildUrl(`/credit-memo?${queryParams}`);
      const response = await fetch(url);

      if (!response.ok) {
        throw new Error(`Failed to fetch credit memos: ${response.status}`);
      }
  ...
  const handleRefresh = () => {
    ...
    const sortDir = gridData.sort?.[0]?.dir || null;
    fetchData(pageNumber, pageSize, debouncedSearchText, statusFilter, sortField, sortDir);
    showNotification('Credit memos refreshed successfully', 'success');
  };
  ...
  // Process credit memos data for the Grid (server-side filtering)
  ...
  const handleView = (id) => {
    navigate(`/credit-memo/view/${id}`);
  };

  // Handle Edit button click
  const handleEdit = (id) => {
    navigate(`/credit-memo/edit/${id}`);
  };
  ...
  const ActionCell = (props) => {
    const creditMemo = props.dataItem;
    if (!creditMemo || !creditMemo.id) {
      return <td {...props.tdProps}>Invalid credit memo data</td>;
    }

    const isClosed = creditMemo.status === 'b2dae51c-12b6-4a90-adfd-4e2345026b70';

    return (
      <td {...props.tdProps} className="k-command-cell">
        <div className="action-buttons">
        <Button
            onClick={() => handleView(creditMemo.id)}
        ...
        {!isClosed && (
        <Button
            onClick={() => handleEdit(creditMemo.id)}
      ...
      <div className="k-p-4">
        <div className="sales-order-list-container error-container">
          <div className="error-message">
            <div>Error loading credit memos: {error}</div>
    ...
    <div className="k-p-4">
      <div className="sales-order-list-container">
        ...
        <div className="list-header">
          <h2>Credit Memo Master</h2>
        <Button
          onClick={() => navigate('/credit-memo/new')}
          className="k-button k-button-md k-rounded-md k-button-solid k-button-solid-primary"
          >
            <FaPlus /> Add New Credit Memo
          </Button>
        </div>
        ...
        {(!data?.results || data.results.length === 0) ? (
          <div className="no-data-message">
            <p>No credit memos found. Click the "Add New Credit Memo" button above to create your first credit memo.</p>
          </div>
        ) : processedData().length === 0 ? (
          <div className="no-data-message">
            <p>No credit memos match your current search criteria. Try adjusting your filters.</p>
          </div>
        ) : (
      <Grid
            data={processedData()}
