...
import { useDebitMemos, useCustomers, useStatus } from '../../hooks/useMasterData';
...
const DebitMemoList = () => {
  ...
  const { debitMemos, loading, error, refresh, fetchWithPagination } = useDebitMemos();
  ...
  const fetchData = useCallback(async (pageNumber = 1, pageSize = 10, search = '', status = 'All', sortField = null, sortDir = null) => {
    try {
      ...
      const url = buildUrl(`/debit-memo?${queryParams}`);
      const response = await fetch(url);

      if (!response.ok) {
        throw new Error(`Failed to fetch debit memos: ${response.status}`);
      }
  ...
  // Trigger search when debounced search text or status filter changes
  useEffect(() => {
    if (currentPage !== 1) {
      setCurrentPage(1);
  ...
  const handleRefresh = () => {
    ...
    const sortDir = gridData.sort?.[0]?.dir || null;
    fetchData(pageNumber, pageSize, debouncedSearchText, statusFilter, sortField, sortDir);
    showNotification('Debit memos refreshed successfully', 'success');
  };

  // Process debit memos data for the Grid (server-side filtering)
  ...
  const handleView = (id) => {
    navigate(`/debit-memo/view/${id}`);
  };

  // Handle Edit button click
  const handleEdit = (id) => {
    navigate(`/debit-memo/edit/${id}`);
  };
  ...
  const CustomerCell = (props) => {
    return (
      <td {...props.tdProps}>
        {props.dataItem.customerName || getCustomerName(props.dataItem.customerID)}
      </td>
    );
  };
  ...
  const StatusCell = (props) => {
    ...
    // Remove any trailing dots or extra characters
  ...
  const ActionCell = (props) => {
    const debitMemo = props.dataItem;
    if (!debitMemo || !debitMemo.id) {
      return <td {...props.tdProps}>Invalid debit memo data</td>;
    }

    const isClosed = debitMemo.status === 'b2dae51c-12b6-4a90-adfd-4e2345026b70';

    return (
      <td {...props.tdProps} className="k-command-cell">
        <div className="action-buttons">
        <Button
            onClick={() => handleView(debitMemo.id)}
        ...
        {!isClosed && (
        <Button
            onClick={() => handleEdit(debitMemo.id)}
      ...
      <div className="k-p-4">
        <div className="sales-order-list-container error-container">
          <div className="error-message">
            <div>Error loading debit memos: {error}</div>
    ...
    <div className="k-p-4">
      <div className="sales-order-list-container">
        ...
        <div className="list-header">
          <h2>Debit Memo Master</h2>
        <Button
          onClick={() => navigate('/debit-memo/new')}
          className="k-button k-button-md k-rounded-md k-button-solid k-button-solid-primary"
          >
            <FaPlus /> Add New Debit Memo
          </Button>
        </div>
        ...
        <div className="filter-section">
          <div className="search-bar">
            ...
            <Input
              placeholder="Search debit memos..."
              value={searchText}
        ...
        {(!data?.results || data.results.length === 0) ? (
          <div className="no-data-message">
            <p>No debit memos found. Click the "Add New Debit Memo" button above to create your first debit memo.</p>
          </div>
        ) : processedData().length === 0 ? (
          <div className="no-data-message">
            <p>No debit memos match your current search criteria. Try adjusting your filters.</p>
          </div>
        ) : (
      <Grid
            data={processedData()}
      ...
      >
            ...
            <GridColumn field="sequenceNumber" title="Transaction Number" width="220px" />
            <GridColumn field="customerID" title="Customer" width="260px" cell={CustomerCell} sortable={false} />
            <GridColumn field="status" title="Status" width="150px" cell={StatusCell} sortable={false} />
            <GridColumn field="totalAmount" title="Amount" width="160px" cell={AmountCell} sortable={false} />
               <GridColumn field="" title="Actions" width="160px" cell={ActionCell} sortable={false} />
      </Grid>
        )}
