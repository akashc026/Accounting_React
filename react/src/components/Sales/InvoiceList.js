...
import { useInvoices, useCustomers, useStatus } from '../../hooks/useMasterData';
...
const InvoiceList = () => {
  ...
  const { invoices, loading, error, refresh, fetchWithPagination } = useInvoices();
  ...
  const fetchData = useCallback(async (pageNumber = 1, pageSize = 10, search = '', status = 'All', sortField = null, sortDir = null) => {
    try {
      ...
      const url = buildUrl(`/invoice?${queryParams}`);
  ...
  const ActionCell = (props) => {
    const invoice = props.dataItem;
    if (!invoice || !invoice.id) {
      return <td {...props.tdProps}>Invalid invoice data</td>;
    }

    const isClosed = invoice.status === 'b2dae51c-12b6-4a90-adfd-4e2345026b70';
      ...
      <div className="k-p-4">
        <div className="sales-order-list-container error-container">
          <div className="error-message">
            <div>Error loading invoices: {error}</div>
      ...
      <div className="sales-order-list-container">
        ...
        {(!data?.results || data.results.length === 0) ? (
          <div className="no-data-message">
            <p>No invoices found. Click the "Add New Invoice" button above to create your first invoice.</p>
          </div>
        ) : processedData().length === 0 ? (
          <div className="no-data-message">
            <p>No invoices match your current search criteria. Try adjusting your filters.</p>
          </div>
        ) : (
      <Grid
            data={processedData()}
