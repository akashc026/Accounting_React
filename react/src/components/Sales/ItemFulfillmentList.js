...
import { useItemFulfillments, useCustomers, useStatus } from '../../hooks/useMasterData';
...
const ItemFulfillmentList = () => {
  ...
  const { itemFulfillments, loading, error, refresh, fetchWithPagination } = useItemFulfillments();
  ...
  const fetchData = useCallback(async (pageNumber = 1, pageSize = 10, search = '', status = 'All', sortField = null, sortDir = null) => {
    try {
      ...
      const url = buildUrl(`/item-fulfilment?${queryParams}`);
      const response = await fetch(url);

      if (!response.ok) {
        throw new Error(`Failed to fetch item fulfillments: ${response.status}`);
      }
      ...
      console.error('Error fetching item fulfillments:', err);
      setData({ results: [] });
      setTotalItems(0);
    }
  }, []);
  ...
  const handleRefresh = () => {
    ...
    showNotification('Item fulfillments refreshed successfully', 'success');
  };
  ...
  const handleView = (id) => {
    navigate(`/item-fulfillment/view/${id}`);
  };

  // Handle Edit button click
  const handleEdit = (id) => {
    navigate(`/item-fulfillment/edit/${id}`);
  };
  ...
  const ActionCell = (props) => {
    const fulfillment = props.dataItem;
    if (!fulfillment || !fulfillment.id) {
      return <td {...props.tdProps}>Invalid fulfillment data</td>;
    }

    const isClosed = fulfillment.status === 'b2dae51c-12b6-4a90-adfd-4e2345026b70';

    return (
      <td {...props.tdProps} className="k-command-cell">
        <div className="action-buttons">
          <Button
            onClick={() => handleView(fulfillment.id)}
          ...
          {!isClosed && (
          <Button
            onClick={() => handleEdit(fulfillment.id)}
      ...
      <div className="k-p-4">
        <div className="sales-order-list-container">
          <div className="loading-indicator">
            ...
            <div>Loading item fulfillments...</div>
          </div>
        </div>
      </div>
    );
      ...
      <div className="k-p-4">
        <div className="sales-order-list-container error-container">
          <div className="error-message">
            <div>Error loading item fulfillments: {error}</div>
    ...
    <div className="k-p-4">
      <div className="sales-order-list-container">
        ...
        <div className="list-header">
          <h2>Item Fulfillment Master</h2>
          <Button
            onClick={() => navigate('/item-fulfillment/new')}
            className="k-button k-button-md k-rounded-md k-button-solid k-button-solid-primary"
          >
            <FaPlus /> Add New Item Fulfillment
          </Button>
        </div>
        ...
        <div className="filter-section">
          <div className="search-bar">
            ...
            <Input
              placeholder="Search item fulfillments..."
              value={searchText}
        ...
        {(!data?.results || data.results.length === 0) ? (
          <div className="no-data-message">
            <p>No item fulfillments found. Click the "Add New Item Fulfillment" button above to create your first item fulfillment.</p>
          </div>
        ) : processedData().length === 0 ? (
          <div className="no-data-message">
            <p>No item fulfillments match your current search criteria. Try adjusting your filters.</p>
          </div>
        ) : (
          <Grid
            data={processedData()}
...
export default ItemFulfillmentList; 