...
import { useSalesOrders, useCustomers, useStatus } from '../../hooks/useMasterData';
...
const SalesOrderList = () => {
  ...
  const { salesOrders, loading, error, refresh, fetchWithPagination } = useSalesOrders();
  ...
  const [gridData, setGridData] = useState({
    ...
    take: 10,
    sort: [
      { field: 'soDate', dir: 'desc' }
    ]
  });

  // Debounce search text with 1000ms delay
  useEffect(() => {
  ...
  const fetchData = useCallback(async (pageNumber = 1, pageSize = 10, search = '', status = 'All', sortField = null, sortDir = null, signal = null) => {
    try {
      ...
      const url = buildUrl(`/sales-order?${queryParams}`);
      ...
      if (err.name === 'AbortError') {
        // Request was aborted - ignore and exit silently
  ...
  const fetchDataWithAbort = useCallback((pageNumber = 1, pageSize = 10, search = '', status = 'All', sortField = null, sortDir = null) => {
    ...
    fetchData(pageNumber, pageSize, search, status, sortField, sortDir, controller.signal);
  }, [fetchData]);
  ...
  useEffect(() => {
    ...
    if (debouncedSearchText !== prevSearchRef.current || statusFilter !== prevStatusRef.current) {
      setGridData(prev => ({ ...prev, skip: 0 }));
      prevSearchRef.current = debouncedSearchText;
      prevStatusRef.current = statusFilter;
    }

    fetchDataWithAbort(1, gridData.take, debouncedSearchText, statusFilter, sortField, sortDir);
  ...
  }, [fetchDataWithAbort, gridData.take, debouncedSearchText, statusFilter, gridData.sort?.[0]?.field, gridData.sort?.[0]?.dir]);
  ...
  const dataStateChange = (e) => {
    ...
    // Check if page, page size, or sort changed
    ...
    if (pageNumber !== currentPage || pageSize !== gridData.take || sortChanged) {
      fetchDataWithAbort(pageNumber, pageSize, debouncedSearchText, statusFilter, sortField, sortDir);
    }
  };
  ...
  const handleRefresh = () => {
    ...
    showNotification('Sales orders refreshed successfully', 'success');
  };
  ...
  const CustomerCell = (props) => {
    return (
      <td {...props.tdProps}>
        {getCustomerName(props.dataItem.customerID)}
      </td>
    );
  };
  ...
  const ActionCell = (props) => {
    const order = props.dataItem;
    if (!order || !order.id) {
      return <td {...props.tdProps}>Invalid order data</td>;
    }

    const isClosed = order.status === 'b2dae51c-12b6-4a90-adfd-4e2345026b70';
  ...
  if (error) {
    return (
      <div className="k-p-4">
        <div className="sales-order-list-container error-container">
          <div className="error-message">
            <div>Error loading sales orders: {error}</div>
    ...
    <div className="k-p-4">
      <div className="sales-order-list-container">
        ...
        <div className="list-header">
          <h2>Sales Order Master</h2>
        <Button
          onClick={() => navigate('/sales-order/new')}
          className="k-button k-button-md k-rounded-md k-button-solid k-button-solid-primary"
          >
            <FaPlus /> Add New Sales Order
          </Button>
        </div>
        ...
        {(!data?.results || data.results.length === 0) ? (
          <div className="no-data-message">
            <p>No sales orders found. Click the "Add New Sales Order" button above to create your first sales order.</p>
          </div>
        ) : processedData().length === 0 ? (
          <div className="no-data-message">
            <p>No sales orders match your current search criteria. Try adjusting your filters.</p>
          </div>
        ) : (
      <Grid
            data={processedData()}
      ...
      >
            ...
            <GridColumn field="customerID" title="Customer" width="200px" cell={CustomerCell} sortable={false} />
            <GridColumn field="status" title="Status" width="150px" cell={StatusCell} sortable={false} />
            <GridColumn field="totalAmount" title="Total Amount" width="140px" cell={AmountCell} sortable={false} />
