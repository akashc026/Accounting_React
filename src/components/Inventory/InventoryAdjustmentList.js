...
const InventoryAdjustmentList = () => {
  ...
  const fetchData = useCallback(async (pageNumber = 1, pageSize = 10) => {
    try {
      ...
      const url = buildUrl(`/inventory-adjustment?PageNumber=${pageNumber}&PageSize=${pageSize}`);
      const response = await fetch(url);

      if (!response.ok) {
        throw new Error(`Failed to fetch inventory adjustments: ${response.status}`);
      }
  ...
  // Filter and process inventory adjustments data for the Grid
  const processedData = useCallback(() => {
    ...
    if (searchText) {
      ...
      filteredData = filteredData.filter(adjustment => {
        const customerName = getCustomerName(adjustment.customerID);
        const searchString = `${adjustment.sequenceNumber || ''} ${customerName} ${adjustment.status}`.toLowerCase();
    ...
    // Apply status filter
    if (statusFilter !== 'All') {
      filteredData = filteredData.filter(adjustment => adjustment.status === statusFilter);
    }
  ...
  const handleView = (id) => {
    navigate(`/inventory-adjustment/view/${id}`);
  };

  // Handle Edit button click
  const handleEdit = (id) => {
    navigate(`/inventory-adjustment/edit/${id}`);
  };
  ...
  const CustomerCell = (props) => {
    return (
      <td {...props.tdProps}>
        {getCustomerName(props.dataItem.customer)}
      </td>
    );
  };
  ...
  const ActionCell = (props) => {
    const adjustment = props.dataItem;
    if (!adjustment || !adjustment.id) {
      return <td {...props.tdProps}>Invalid adjustment data</td>;
    }
  ...
  if (loading) {
    return (
      <div className="k-p-4">
        <div className="inventory-adjustment-list-container">
          <div className="loading-indicator">
            <div className="spinner"></div>
            <div>Loading inventory adjustments...</div>
          </div>
        </div>
      </div>
    );
  ...
  if (error) {
    return (
      <div className="k-p-4">
        <div className="inventory-adjustment-list-container error-container">
          <div className="error-message">
            <div>Error loading inventory adjustments: {error}</div>
    ...
    <div className="k-p-4">
      <div className="inventory-adjustment-list-container">
        ...
        <div className="list-header">
          <h2>Inventory Adjustment Master</h2>
        <Button
          onClick={() => navigate('/inventory-adjustment/new')}
          className="k-button k-button-md k-rounded-md k-button-solid k-button-solid-primary"
          >
            <FaPlus /> Add New Inventory Adjustment
          </Button>
        </div>
        ...
        {(!data?.results || data.results.length === 0) ? (
          <div className="no-data-message">
            <p>No inventory adjustments found. Click the "Add New Inventory Adjustment" button above to create your first inventory adjustment.</p>
          </div>
        ) : processedData().length === 0 ? (
          <div className="no-data-message">
            <p>No inventory adjustments match your current search criteria. Try adjusting your filters.</p>
          </div>
        ) : (
      <Grid
            data={processedData()}
