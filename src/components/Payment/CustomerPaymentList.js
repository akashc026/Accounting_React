...
const CustomerPaymentList = () => {
  ...
  // Filter and process customer payments data for the Grid
  const processedData = useCallback(() => {
    ...
    if (searchText) {
      filteredData = filteredData.filter(payment => {
        const customerName = payment.customerName || getCustomerName(payment.customerID);
        const searchString = `${payment.sequenceNumber || ''} ${customerName} ${formatAmount(payment.amount)}`.toLowerCase();
