...
import { useVendorPayments, useVendors, useStatus } from '../../hooks/useMasterData';
...
const VendorPaymentList = () => {
  ...
  const { vendorPayments, loading, error, refresh, fetchWithPagination } = useVendorPayments();
  ...
  // Filter and process vendor payments data for the Grid
  const processedData = useCallback(() => {
    ...
    if (searchText) {
      filteredData = filteredData.filter(payment => {
        const vendorName = payment.vendorName || getVendorName(payment.vendorID);
        const searchString = `${payment.sequenceNumber || ''} ${vendorName} ${formatAmount(payment.totalAmount)}`.toLowerCase();
    ...
    if (statusFilter !== 'All') {
      filteredData = filteredData.filter(payment => {
        const statusName = getStatusName(payment.status).replace(/\.+$/, '').trim();
  ...
  const handleView = (id) => {
    navigate(`/vendor-payment/view/${id}`);
  };

  // Handle Edit button click
  const handleEdit = (id) => {
    navigate(`/vendor-payment/edit/${id}`);
  };
  ...
  const VendorCell = (props) => {
    return (
      <td {...props.tdProps}>
        {props.dataItem.vendorName || getVendorName(props.dataItem.vendorID)}
      </td>
    );
  };
      ...
      <div className="k-p-4">
        <div className="payment-list-container error-container">
          <div className="error-message">
            <div>Error loading vendor payments: {error}</div>
            <Button type="button" onClick={handleRefresh} className="k-button k-button-md k-rounded-md k-button-solid k-button-solid-primary">
              <FaSync /> Retry
            </Button>
          </div>
        </div>
    ...
    <div className="k-p-4">
      <div className="payment-list-container">
        ...
        <div className="list-header">
          <h2>Vendor Payment Master</h2>
        <Button
          type="button"
          onClick={() => navigate('/vendor-payment/new')}
          className="k-button k-button-md k-rounded-md k-button-solid k-button-solid-primary"
          >
            <FaPlus /> Add New Vendor Payment
          </Button>
        </div>
        ...
        {(!data?.results || data.results.length === 0) ? (
          <div className="no-data-message">
            <p>No vendor payments found. Click the "Add New Vendor Payment" button above to create your first vendor payment.</p>
          </div>
        ) : processedData().length === 0 ? (
          <div className="no-data-message">
            <p>No vendor payments match your current search criteria. Try adjusting your filters.</p>
          </div>
        ) : (
      <Grid
            data={processedData()}
