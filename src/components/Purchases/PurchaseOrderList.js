...
const PurchaseOrderList = () => {
  ...
  // Filter and process purchase orders data for the Grid
  const processedData = useCallback(() => {
    ...
    if (searchText) {
      filteredData = filteredData.filter(order => {
        const vendorName = order.vendorName || getVendorName(order.vendorID);
        const searchString = `${order.sequenceNumber || ''} ${vendorName} ${order.status} ${formatAmount(order.totalAmount)}`.toLowerCase();
    ...
    if (statusFilter !== 'All') {
      filteredData = filteredData.filter(order => {
        const statusName = getStatusName(order.status).replace(/\.+$/, '').trim();
