...
const VendorBillList = () => {
  ...
  // Filter and process vendor bills data for the Grid
  const processedData = useCallback(() => {
    ...
    if (searchText) {
      filteredData = filteredData.filter(bill => {
        const vendorName = getVendorName(bill.vendorID);
        const searchString = `${bill.sequenceNumber || ''} ${vendorName} ${bill.status} ${formatAmount(bill.totalAmount)}`.toLowerCase();
    ...
    if (statusFilter !== 'All') {
      filteredData = filteredData.filter(bill => {
        const statusName = getStatusName(bill.status).replace(/\.+$/, '').trim();
