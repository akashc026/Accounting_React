...
const VendorCreditList = () => {
  ...
  // Filter and process vendor credits data for the Grid
  const processedData = useCallback(() => {
    ...
    if (searchText) {
      filteredData = filteredData.filter(credit => {
        const vendorName = getVendorName(credit.vendorID);
        const searchString = `${credit.sequenceNumber || ''} ${vendorName} ${formatAmount(credit.totalAmount)}`.toLowerCase();
    ...
    if (statusFilter !== 'All') {
      filteredData = filteredData.filter(credit => {
        const statusName = getStatusName(credit.status).replace(/\.+$/, '').trim();
