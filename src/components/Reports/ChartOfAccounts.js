        ...
        <Card className="k-shadow-lg" style={{ borderRadius: '12px', overflow: 'hidden' }}>
          <CardBody>
            <div className="k-d-flex k-justify-content-center k-align-items-center" style={{ minHeight: '400px' }}>
              <div className="k-d-flex k-flex-col k-align-items-center k-gap-3">
                <div className="k-loader k-loader-lg"></div>
                <p className="k-text-muted">Loading Chart of Accounts...</p>
              </div>
            </div>
          </CardBody>
        </Card>
        ...
        >
          ...
          <div className="k-d-flex k-gap-2">
            {Object.entries(summary).map(([type, count]) => (
              <div key={type} className={`k-badge k-badge-rounded ${
                type.toLowerCase() === 'asset' ? 'k-badge-success' :
                type.toLowerCase() === 'liability' ? 'k-badge-warning' :
                type.toLowerCase() === 'equity' ? 'k-badge-info' :
                type.toLowerCase() === 'income' || type.toLowerCase() === 'revenue' ? 'k-badge-primary' :
                type.toLowerCase() === 'expense' ? 'k-badge-error' : 'k-badge-secondary'
              }`}>
                <span className={`k-icon ${
                  type.toLowerCase() === 'asset' ? 'k-i-dollar' :
                  type.toLowerCase() === 'liability' ? 'k-i-warning' :
                  type.toLowerCase() === 'equity' ? 'k-i-user' :
                  type.toLowerCase() === 'income' || type.toLowerCase() === 'revenue' ? 'k-i-plus' :
                  type.toLowerCase() === 'expense' ? 'k-i-minus' : 'k-i-file'
                } k-mr-1`}></span>
                {type}: {count}
              </div>
            ))}
