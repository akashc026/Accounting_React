...
const CreditMemoList = () => {
  ...
  // Filter and process credit memos data for the Grid
  const processedData = useCallback(() => {
    ...
    // Apply search filter
    if (searchText) {
      filteredData = filteredData.filter(creditMemo => {
        const customerName = creditMemo.customerName || getCustomerName(creditMemo.customerID);
        const searchString = `${creditMemo.sequenceNumber || ''} ${customerName} ${formatAmount(creditMemo.totalAmount)}`.toLowerCase();
    ...
    if (statusFilter !== 'All') {
      filteredData = filteredData.filter(creditMemo => {
        const statusName = getStatusName(creditMemo.status).replace(/\.+$/, '').trim();
