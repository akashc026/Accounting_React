...
const DebitMemoList = () => {
  ...
  // Filter and process debit memos data for the Grid
  const processedData = useCallback(() => {
    ...
    // Apply search filter
    if (searchText) {
      filteredData = filteredData.filter(debitMemo => {
        const customerName = debitMemo.customerName || getCustomerName(debitMemo.customerID);
        const searchString = `${debitMemo.sequenceNumber || ''} ${customerName} ${formatAmount(debitMemo.totalAmount)}`.toLowerCase();
    ...
    if (statusFilter !== 'All') {
      filteredData = filteredData.filter(debitMemo => {
        const statusName = getStatusName(debitMemo.status).replace(/\.+$/, '').trim();
  ...
  return (
    <div className="k-p-4">
      ...
      >
            ...
            <GridColumn field="customerID" title="Customer" width="260px" cell={CustomerCell} />
            <GridColumn field="status" title="Status" width="150px" cell={StatusCell} />
            <GridColumn field="totalAmount" title="Amount" width="160px" cell={AmountCell} />
               <GridColumn field="" title="Actions" width="160px" cell={ActionCell} />
      </Grid>
        )}
