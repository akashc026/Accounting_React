...
const InvoiceList = () => {
  ...
  // Filter and process invoices data for the Grid
  const processedData = useCallback(() => {
    ...
    if (searchText) {
      filteredData = filteredData.filter(invoice => {
        ...
        const searchString = `${invoice.invoiceNumber || ''} ${customerName} ${invoice.status} ${formatAmount(invoice.totalAmount)}`.toLowerCase();
    ...
    if (statusFilter !== 'All') {
      filteredData = filteredData.filter(invoice => {
        const statusName = getStatusName(invoice.status).replace(/\.+$/, '').trim();
