...
const ItemFulfillmentList = () => {
  ...
  // Filter and process item fulfillments data for the Grid
  const processedData = useCallback(() => {
    ...
    // Apply search filter
    if (searchText) {
      filteredData = filteredData.filter(fulfillment => {
        const customerName = getCustomerName(fulfillment.customerID);
        const searchString = `${customerName} ${fulfillment.sequenceNumber || ''}`.toLowerCase();
    ...
    if (statusFilter !== 'All') {
      filteredData = filteredData.filter(fulfillment => {
        const statusName = getStatusName(fulfillment.status).replace(/\.+$/, '').trim();
